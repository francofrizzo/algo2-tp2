...
\subsection{TAD Arreglo dimensionable($\alpha$)}
  ...
  \tadAxioma{fin($a$)}{\IF (tam(a) $\leq$ 1) THEN crearArreglo(0) ELSE fin\textsubscript{aux}($a$, crearArreglo(tam($a$)-1), tam($a$)-2) FI}

  \tadOperacion{fin\textsubscript{aux}}{ad($\alpha$)/a, ad($\alpha$)/b, nat}{ad($\alpha$)}{definido?($a$, $n$) $\land$ definido?($b$, $n - 1$)}
  \tadAxioma{fin\textsubscript{aux}($a$, $b$, $n$)}{\IF $n$ = 0 THEN $b$[0] $\leftarrow$ $a$[1] ELSE fin\textsubscript{aux}($a$, $b$[$n$] $\leftarrow$ $a$[$n$ + 1], $n$ - 1) FI}
