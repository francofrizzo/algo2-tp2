...
\begin{Algoritmos}
  ...
  \begin{contAlgoritmo}{iBorrar}
    \Else{
      ...
      \eIf(\com*[f]{$\Theta(1)$}){\esNil{\izq{$reemplazo$}}}{
        \eIf(\com*[f]{$\Theta(1)$}){$\neg$ \esNil{$padre$}}{
          \eIf(\com*[f]{$\Theta(1)$}){\izq{$padre$} = $lugar$}{
            \izq{$padre$} $\gets reemplazo$ \com*{$\Theta(1)$}
          }{
            \der{$padre$} $\gets reemplazo$ \com*{$\Theta(1)$}
          }
        }{
          $a \gets reemplazo$ \com*{$\Theta(1)$}
        }
        \raiz{$reemplazo$}.$padre \gets padre$ \com*{$\Theta(1)$}
        \izq{$reemplazo$} $\gets$ \izq{$lugar$} \com*{$\Theta(1)$}
        \izq{$lugar$}.$padre$ $\gets reemplazo$ \com*{$\Theta(1)$}
        ...
        \eIf(\com*[f]{$\Theta(1)$}){$\neg$ \esNil{$padre$}}{
          \eIf(\com*[f]{$\Theta(1)$}){\izq{$padre$} = $lugar$}{
            $padre \gets$ \bin{$reemplazo$, \raiz{$padre$}, \der{$padre$}}
          }{
            $padre \gets$ \bin{\izq{$padre$}, \raiz{$padre$}, $reemplazo$}
          }
        }{
