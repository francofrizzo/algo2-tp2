...
\begin{Representacion}
  ...
  \Rep[estrAVL][e]{
    ...
    $(\paratodo{nodo}{n})$ ($n$ $\in$ nodos($e$) $\impluego$ $(*(n.izq) \neq *(n.der)$ $\land$ \\
...
\begin{Algoritmos}
  ...
  \begin{algoritmo}{iDefinir}{\Inout{e}{estrAVL}, \In{k}{$\kappa$}, \In{s}{$\sigma$}}{}
    ...
    \eIf{$lugar \neq \NULL$}{
      ...
      \tipo{puntero(nodo)} $nuevo \gets \new \langle k$, $s$, NULL, NULL, NULL, $1 \rangle$ \tcp{Reservamos memoria para el nuevo nodo} \com*{$\Theta(copy(k) + copy(s))$}
  ...
  \begin{algoritmo}{iRotarAIzquierda}{\In{n}{puntero(nodo)}}{}
    \If(\com*[f]{$\Theta(1)$}){$n.padre \neq \NULL$}{
      \eIf(\com*[f]{$\Theta(1)$}){$n.padre \to izq = n$}{
        $(n.padre \to izq) \gets n.der$ \com*{$\Theta(1)$}
      }{
        $(n.padre \to der) \gets n.der$ \com*{$\Theta(1)$}
      }
    }
    $(n.der \to padre) \gets n.padre$ \com*{$\Theta(1)$} 
    $n.padre \gets n.der$ \com*{$\Theta(1)$}
    $n.der \gets (n.der \to izq)$ \com*{$\Theta(1)$}
    \If(\com*[f]{$\Theta(1)$}){$n.der \neq \NULL$}{
      $(n.der \to padre) \gets n$ \com*{$\Theta(1)$}
    }
    $(n.padre \to izq) \gets n$ \com*{$\Theta(1)$}
    % \recalcAlts{$n$} \com*{$\Theta(\log(n) + order(k))$}
  \end{algoritmo}
  \complejidad{$\Theta(1)$}
