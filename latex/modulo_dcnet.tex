...
\item	($\forall L$: nat)( 0 $\leq$ $L$ < tam($e$.paqEnEspera) $\impluego$ (
      ...
      ($\forall$ $it_1$: ItConj(paquete)) $it$ $\in$ dame$\Pi_1$(juntarSignificados($\Pi_2$($e$.paquetesEnEspera[$L$]))) $\impluego$ haySiguiente?($it_1$) $\land$ \\

      ($\forall$ $it_2$: ItConj(paquete)) $it_2$ $\in$ dame$\Pi_2$(colaAConj($\Pi_3$($e$.paquetesEnEspera[$L$]))) $\impluego$ haySiguiente?($it_2$) $\yluego$ \\
			...
			($\forall c$: ID) (c $\in$ claves($\Pi_2$($e$.paquetesEnEspera[$L$]))) $\impluego$ $\Pi_1$(siguiente($\Pi_1$(obtener(c, $\Pi_2$($e$.paquetesEnEspera[$L$]))))) = c $\land$ \\

      ($\forall$ $it$:ItConj(paquete)) siguiente($it$) $\in$ dameSiguientes(dame$\Pi_2$(colaAConj($\Pi_3$($e$.paquetesEnEspera[$L$])))) $\impluego$ siguiente($it$) $\in$ $\Pi_1$($e$.paquetesEnEspera[$L$]) \\
