...
\begin{enumerate} 
%1
\item (tam($e$.$paquetesEnEspera$) = tam($e$.$IPsCompusPorID$) = tam($e$.$siguientesCompus$) = tam($e$.$\#PaqEnviados$) = \#(computadoras($e$.$red$))) $\land$ 
    ...
    \While(\com*[f]{$\Theta(n)$}){$j$ < Cardinal(Computadoras($d$.red))}{
      \If(\com*[f]{$\Theta(1)$}){!(EsVacio?($d$.$paquetesEnEspera$[$j$]).$enConjunto$)}{
        ...
        \If(\com*[f]{$\Theta(1)$}){!($d$.siquienteCompu[$j$][$dest$] = $dest$)}{
          ...
          $it \gets$ AgregarRapido(($d$.$paquetesEnEspera$[$d$.$siguientesCompus$[$j$][$dest$]]).$enConjunto$, $p$)\com*{$\Theta(I)$ lo tomamos como $\Theta(1)$}
          Definir($d$.$paquetesEnEspera$[$d$.$siguientesCompus$[$j$][$dest$]].porID, $p$.ID, $\langle$ $it$, $d$.$siguientesCompus$[$j$][$dest$], $dest$, $\rangle$)\com*{$\Theta(log(k))$}
