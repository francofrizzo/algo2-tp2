...
%    }
  ...
  \Encabezado{Operaciones del heap}
    ...
    \InterfazFuncion{vac\'{i}a}{}{heap($\alpha$)}
%    \InterfazFuncion{heap}{}{heap($\alpha$)}
    [true]
    {$res \igobs$ vac\'{i}a}
    [$\Theta(1)$]
    ...
    \InterfazFuncion{vac\'{i}a?}{\In{h}{heap($\alpha$)}}{bool}
    [true]
    {$h \igobs$ vac\'{i}a()}
    [$\Theta(1)$]
...
\begin{Representacion}
  ...
  \begin{Estructura}{heap($\alpha$)}[hp]

%
% NOSE COMO SE ESCRIVE ESTO.. DEVERIA DECIR
%   Genero: heap($\alpha$) se representa con hp
%       donde hp es vector($\alpha$)
%

    \begin{vector($\alpha$)}[hp]
    \end{vector}
  ...
  \AbsFc[heap($\alpha$)]{colaPrior($\alpha$)}[h]{
...
%  \begin{algoritmo}{iDesencolar}{\Inout{hp}{heap($\alpha$)}}{$\alpha$}
  \begin{algoritmo}{iDesencolar}{\Inout{hp}{heap($\alpha$)}}{}
    ...
    \tipo{nat} $i \gets$ $hp$.tama\~{n}o - 1\;
    iSwap($hp$[$0$], $hp$[$i$])\;
%    $res \gets$ $hp$.pop_back()\;
    $hp$.pop_back()\;

    \tipo{nat} $hijo_{0} \gets$ 0\;
    \tipo{nat} $hijo_{1} \gets$ 1\;

    \While{$hijo_{0}$ < $hp$.tama\~{n}o $\land$ $hijo_{1}$ < $hp$.tama\~{n}o $\yluego$ ($hp$[$hijo_{0}$] < $hp$[$i$] $\lor$ $hp$[$hijo_{1}$] < $hp$[$i$])}{
