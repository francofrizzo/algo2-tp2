...
\begin{Representacion}
  ...
  % Hay que rehacer el REP, y explicarlo en castellano
  ...
  \tadAxioma{DevolverInterfaz($l$, $c_{1}$, $c_{2}$)}{\IF ($\Pi_{1}$(prim($l$)) = $\Pi_{1}$($c_{1}$)) THEN DevolverInterfaz\textsubscript{aux}($\Pi_{2}$(prim($l$), $c_{2}$)) ELSE DevolverInterfaz(fin($l$, $c_{1}$, $c_{2}$)) FI}

  ~  
  
  \tadOperacion{DevolverInterfaz\textsubscript{aux}}{secu(tupla(Interfaz, ItRed))/l, compu/c}{Interfaz}{} % Restriccion? Pertenece?(l, c1, c2) o c1 \in computadoras(r)
  \tadAxioma{DevolverInterfaz($l$, $c$)}{\IF ($\Pi_{1}$($c_{2}$) = $\Pi_{1}$(siguiente($\Pi_{2}$(prim($l$))))) THEN $\Pi_{1}$(prim($l$)) ELSE DevolverInterfaz\textsubscript{aux}(fin($l$, c)) FI}
...
\begin{Algoritmos}
  ...
  \begin{algoritmo}{iConectar}{\Inout{r}{estrRed)}, \In{c_{1}}{compu}, \In{i_{1}}{interfaz}, \In{c_{2}}{compu}, \In{i_{2}}{interfaz}}{}
    \tipo{itDicc(IP, diccConexiones)} $it_1 \gets$ \crearIt{$r.conexiones$} \com*{$\Theta(1)$}
    \tipo{itDicc(IP, diccConexiones)} $it_2 \gets$ \crearIt{$r.conexiones$} \com*{$\Theta(1)$}
    \While(\com*[f]{$\Theta(n)$ iteraciones}){\sigClave{$it_1$} $\neq$ $c_1.IP$}{
      \avanzar{$it_1$} \com*{$\Theta(1)$}
    }
    \While(\com*[f]{$\Theta(n)$ iteraciones}){\sigClave{$it_2$} $\neq$ $c_1.IP$}{
      \avanzar{$it_2$} \com*{$\Theta(1)$}
    }
    \defRap{\sigSignif{$it_1$}, $i_1$, \copiar{$it_2$}} \com*{$\Theta(1)$}
    \defRap{\sigSignif{$it_2$}, $i_2$, \copiar{$it_1$}} \com*{$\Theta(1)$}
  \end{algoritmo}
  \datosAlgoritmo{} % Descripción
  {} % Pre
  {} % Post
  {$\Theta(n)$} % Complejidad
  {El algoritmo tiene dos ciclos que se ejecutan $\Theta(n)$ veces, cada una con complejidad $\Theta(1)$. El resto de las operaciones tiene complejidad $\Theta(1)$.} % Justificacíón
  ...
  \begin{algoritmo}{iConectadas?}{\In{r}{estrRed}, \In{c_{1}}{compu}, \In{c_{2}}{compu}}{bool}{}
    ...
    \tipo{itDicc(interfaz, itDicc(IP, diccConexiones))} $it_2 \gets$ \crearIt{\signif{\sig{$it_1$}}} \com*{$\Theta(1)$}
    \While(\com*[f]{$\Theta(I)$ iteraciones}){\haySig{$it_2$} $\yluego$ \sigClave{\sigSignif{$it_2$}} $\neq$ $c_2.IP$}{
      \avanzar{$it_2$} \com*{$\Theta(1)$}
    }
    $res \gets$ \haySig{$it_2$} $\yluego$ \sigClave{\sigSignif{$it_2$}} = $c_2.IP$ \com*{$\Theta(1)$}
  \end{algoritmo}  
  \datosAlgoritmo{} % Descripción
  {} % Pre
  {} % Post
  {$\Theta(n+I)$} % Complejidad
  {El algoritmo tiene dos ciclos; uno de ellos se ejecuta $\Theta(n)$ veces, y el otro $\Theta(I)$ veces, todas ellas con complejidad $\Theta(1)$. El resto de las operaciones tiene complejidad $\Theta(1)$.} % Justificacíón
