...
\begin{Interfaz}
  ...
  \Encabezado{Operaciones de diccionario}
    ...
    \InterfazFuncion{vac\'{i}o}{}{diccStr($\alpha$)}
    [true]
    {$res \igobs$ vac\'{i}o}
    [$\Theta(1)$]
    [Devuelve un diccionario vac\'io.]

    \InterfazFuncion{definir}{\Inout{d}{diccStr($\alpha$)}, \In{k}{string}, \In{s}{$\alpha$}}{diccStr($\alpha$)}
    [$d \igobs d_{0}$]
    {$d \igobs$ definir($k$, $s$, $d_{0}$}
    [$\Theta(L)$]
    [Define una palabra en el diccionario.]

    \InterfazFuncion{borrar}{\Inout{d}{diccStr($\alpha$)}, \In{k}{string}}{diccStr($\alpha$)}
    [$d \igobs d_{0}$ $\land$ def?($d$, $k$)]
    {$d \igobs$ borrar($k$, $s$, $d_{0}$}
    [$\Theta(L)$]
    [Borra una palabra del diccionario.]

    \InterfazFuncion{def?}{\In{d}{diccStr($\alpha$)}, \In{k}{string}}{bool}
    [true]
    {$res \igobs$ def?($d$, $k$)}
    [$\Theta(L)$]
    [Devuelve true si y solo si la clave $k$ est\'a definida en el diccionario.]

    \InterfazFuncion{obtener}{\In{d}{diccStr($\alpha$)}, \In{k}{string}}{$\alpha$}
    [def?($d$, $k$)]
